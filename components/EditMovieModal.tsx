...
export default function EditMovieModal({ movie, onClose, onUpdated }: Props) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(movie.poster_url || null)
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase
      .from("movies")
      .update({ ...form, poster_url })
      .eq("id", movie.id)

    if (error) {
      alert("Gagal update film: " + error.message)
    } else {
      onUpdated()
      onClose()
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-zinc-900 rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl my-auto">
        <h2 className="text-white text-xl font-bold">✏️ Edit Film</h2>
